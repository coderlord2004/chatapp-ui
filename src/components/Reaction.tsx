	...
	return (
		...
		>
			{showReactionMenu && (
				<div className="absolute top-[-100%] left-0 flex gap-2 rounded-[30px] bg-gray-800 p-2 transition-all duration-300">
					{reactionTypes.map((reactionType, index) => (
						<button
							key={index}
							className="animate-fadeInUp cursor-pointer text-lg transition-all duration-200 hover:scale-125"
			...
			<div
				...
				style={{
					backgroundColor: postData.reacted
						? 'rgba(59, 130, 246, 0.1)'
						: 'transparent',
			...
			>
				...
				{reaction.charAt(0).toUpperCase() + reaction.toLowerCase().slice(1)}
			</div>
		</div>
	);
}
