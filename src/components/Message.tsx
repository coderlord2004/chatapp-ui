...
type Props = {
    ...
    updateMessage: (messageId: number, newMessage: string, sending: boolean, isUpdated: boolean,) => void,
...
export default function Message({ index, message, totalMessages, uploadProgress, updateMessage, deleteMessage }: Props) {
            ...
            >
                {message.message && (
                    ...
                    >
                        ...
                        <p
                            className={`absolute top-[100%] ${isAuthUser(message.sender) ? 'right-0' : 'left-0'} text-[60%] whitespace-nowrap text-gray-500 ${index === totalMessages - 1 ? '' : 'hidden group-hover:block'}`}
                        >
                            {message.sending
                                ? 'Đang gửi'
